...
;v4.71  - Stack contents changes and bug fix
;v4.71 VCAL 
;		- Added the ability to "re-route" one of the VCAL addresses for use as a debugger call, allowing 1-Byte calls to debugger for 100% BP address compatibility
;4.8   VCAL
;		- Code optimization for smaller ROM footprint
;v4.9   - Added compaitibility options for interchanging P11 P12 P13 P14 P39 and P51 ECUs
;v5.0	- Added 4 more bytes to datastream for extra RAM
...
;Byte 34 - SSP rev
;Byte 35 - SSP
;Byte 36 - Stack1 rev (Top of stack contents, Current RT address or PSW for ISR RTI))
;Byte 37 - Stack1 
;Byte 38 - Stack2 rev (LRB for ISR RTI)
;Byte 39 - Stack2 
;Byte 40 - Stack3 rev (ACC for ISR RTI)
;Byte 41 - Stack3
;Byte 42 - Stack4 rev (RT for ISR RTI) 
;Byte 43 - Stack4
;Byte 44 - Stack5 
;Byte 45 - Stack5
;Byte 46 - 0xDE 
...
;-------------- P11 P12 P13 P14 P39 and P51 Code Options --------------------------
...
;SCBValue				equ		002h	;(0HH,LLh) PSW value to use for debugger code
...
;PatchAddress3			equ		06d67h  ;Compatibility patch,allows p13 code on p13
;Patch3					equ		012h    ;Compatibility patch,allows p13 code on p13
;PatchAddress4			equ		06d68h  ;Compatibility patch,allows p13 code on p13
;Patch4					equ		013h    ;Compatibility patch,allows p13 code on p13
;PatchAddress5			equ		06d59h  ;Compatibility patch,allows p13 code on p13-n01
;Patch5					equ		006h    ;Compatibility patch,allows p13 code on p13-n01
;PatchAddress6			equ		06d5ah  ;Compatibility patch,allows p13 code on p13-n01
;Patch6					equ		005h    ;Compatibility patch,allows p13 code on p13-n01
;PatchAddress7			equ		06d60h  ;Compatibility patch,allows p13 code on p51
;Patch7					equ		00fh    ;Compatibility patch,allows p13 code on p51
;PatchAddress8			equ		06d64h  ;Compatibility patch,allows p13 code on p51
;Patch8					equ		010h    ;Compatibility patch,allows p13 code on p51
;PatchAddress9			equ		06d69h  ;Compatibility patch,allows p13 code on p11
;Patch9					equ		001h    ;Compatibility patch,allows p13 code on p11
;PatchAddress10			equ		06d6ah  ;Compatibility patch,allows p13 code on p14
;Patch10					equ		007h    ;Compatibility patch,allows p13 code on p14
;PatchAddress11			equ		06d6bh  ;Compatibility patch,allows p13 code on p14
;Patch11					equ		008h    ;Compatibility patch,allows p13 code on p14
;PatchAddress12			equ		06d6ch  ;Compatibility patch,allows p13 code on p14
;Patch12					equ		009h    ;Compatibility patch,allows p13 code on p14
;PatchAddress13			equ		06d6eh  ;Compatibility patch,allows p13 code on p39
;Patch13					equ		00bh    ;Compatibility patch,allows p13 code on p39
;PatchAddress14			equ		00d22h	;Checksum patch
;Patch14					equ		003h    ;Checksum patch
;PatchAddress15			equ		00d23h  ;Checksum patch
;Patch15					equ		036h    ;Checksum patch
;PatchAddress16			equ		00d24h  ;Checksum patch
;Patch16					equ		00dh    ;Checksum patch
;PatchAddress17			equ		00d25h  ;Checksum patch
;Patch17					equ		000h    ;Checksum patch
;PatchAddress18			equ		009dbh  ;Checksum patch
;Patch18					equ		0cbh    ;Checksum patch
;PatchAddress19			equ		00a27h  ;Force J8 Serial Jumper
;Patch19					equ		085h    ;Force J8 Serial Jumper
;PatchAddress20			equ		0104fh  ;Force J8 Serial Jumper
;Patch20					equ		085h    ;Force J8 Serial Jumper
;
;;-------------- HTS Code Options --------------------------
...
;Do not change the order or remove anything without making a change to the PC app
				...
				DW PatchBytes			;Location of Patch list
				DW DebuggerStart 		;Length of option bytes
				DW DebuggerCodeAddress  ;Default location to place debugger code in ROM
				DW FirstBaudRateAddy	;First Address in ROM to patch baud rate
				DW SecondBaudRateAddy   ;Second Address in ROM to patch baud rate
				DB BaudValue			;Baud rate byte to patch
				DW WhichLoop    		;WhichLoop Byte
				DW DebuggerLRB +1 		;Debugger LRB address
				DW DebuggerPSW +2 		;Debugger PSW address
				DW DebuggerIE +3  		;Debugger IE address
				DW Ram1 				;Extra Ram Address 1
				DW Ram2 				;Extra Ram Address 2
				DW Ram3 				;Extra Ram Address 3
				DW Ram4 				;Extra Ram Address 4
				DW Ram5 				;Extra Ram Address 5
				DW Ram6 				;Extra Ram Address 6
				DW Ram7 				;Extra Ram Address 7
				DW Ram8 				;Extra Ram Address 8
				DW Ram9 				;Extra Ram Address 9
				DW Ram10 				;Extra Ram Address 10
				DW realVcal + 1			;Orig VCAL Address
				DW whiteList			;WhiteList location
...
;### Patches will not work unless listed here!! #####

;If patches are listed as ADDRESS EQU PATCH up top, they must still be activated here
...
DebuggerStart:	
;---------------------- Register Snapshot ----------------------------------
;Push the ECU state to the stack, so it can be restored via RTI command
;Push the pointer registers to the stack so they can be reported via the debugger

				PUSHS 	A						;Push a placeholder to the stack for possible return address for whitelist caller
...
DebuggerPSW:	MOVB    PSWL, #SCBValue   		;Change DP, X1, X2 registers to debugger registers 
				

;------------- VCAL Re route processing ------------------------------------
		
				SCAL 	getDBAddressJump1		;Get the ROM address for the start of the DB area in X1
				INC 	X1						;Adjust the db address pointer so it points to the VCAL whitelist
				ADD 	SSP, #0000eh			;Point the stack at the return address from the VCAL instruction
				POPS 	A						;Return address in A
				PUSHS 	A 						;Push this address back incase a whitelist caller needs it for VCAL RT
				MOV 	X2, A
				DEC 	X2						;X2 now contains the address that VCAL was called FROM 
realVcal:		L		A, #00000h				;Load the location of the orig VCAL routine into A (PC app modifies this value)
				PUSHS   A						;Push vcal address to stack to be used as a return
				
												;Check Whitelist for real VCALers, loop until 0 
chkWhiteList:	LC 		A, [X1]					;Address from whiteList
				JEQ		contDBGR				;JEQ - End of list reached, this was a debugger call
				CMP		X2, A					;Check to see if the caller was on the whiteList
				JEQ     exitDBGR				;Caller was on the list, this was an actual VCAL
				INC		X1
				INC 	X1
				SJ		chkWhiteList
...
exitDBGR:		
				SUB 	SSP, #00006h			;Adjust the stack to where it should be (ready to POP X1)
				RTI								;Restore the state of the ECU and jump into the "real" VCAL routine
				
;----------------Continue debugger code here -------------------------------
contDBGR:		
				POPS 	A 						;Adjust stack so it's pointing at original RT
				POPS 	A
				L 		A, X2					;Push caller address back to stack
				PUSHS 	A						;Push the caller address to the RT position on the stack, when this debugger code was called
				SUB 	SSP, #0000eh			;Adjust SSP so it's ready to pop DP
				L   	A, er3
...
;Change LRB so the registers can be used without affecting the code being debugged
;Then Pop all the preserved registers and send via serial

DebuggerLRB:	MOV     LRB, #LRBValue			;Change er# registers
				CLR 	er3                     ;Clear er3 so r6 and r7 can be used for byte count and checksum
				SCAL 	SendWord				;Send er0
				MOVB 	r3, #001h				;Feeder count
				...
				POPS 	A						;Discard the whitelist placeholder on the stack
				...
				SJ 		GetRamtable	
;--------------------Jump extension ----------------------------------------
;The jump extensions are necesary to allow the use of offset jumps. 
;They allow the code to be moved freely around the ROM without re-compiling 
Feeder2Extension:	SJ		WDT_feeder2
getDBAddressJump1:  SJ		GetDBAddressInX1    
;----------------------------- Serial --------------------------------------
...
;----------------- Send tail bytes, Byte count, and checksum ---------------
...
GetRamtable:
				SCAL	GetDBAddressInX1				;Get the location of WhichLoop DB in A and X1
				SUB 	A, #((WhichLoop - Ram1) + 2)	;Adjust pointer to (RAM Table -2)
				MOV 	X1, A							
GetRam:			INC 	X1								;Loop through RAM table sending byte contents until 0000 pointer is reached
				INC 	X1		
				LC 		A, [X1]
				JEQ		SendSSPandStack
				MOV 	DP, A
...
SendSSPandStack:
				MOV 	X1, SSP					;X1 = Adjusted SSP to what the debugee sees
				INC 	X1
				INC 	X1
				L 		A, X1
				SCAl 	SwapThenSend            ;Send the Stack pointer
				AND 	X1, #0fffeh				;Make sure we have an even address to respect the word boundary. 66911 stack handling makes this necessary.
				MOV 	X2, #00005h				;Set the stack contents limit to send
												;Offset the stack address since we aren't "popping" values
SendStack:		
				INC 	X1                      ;Loop through sending stack contents using X1 as an increasing SSP 
				INC 	X1                      
				CMP 	X1, #(StackBottom & 0fffeh)		
				JGT 	DoneSending				;Stop sending if the stack is at the bottom
                L 	    A, 00000h[X1]
				SCAL 	SwapThenSend
				DEC 	X2						;Stack Counter --
                JEQ		DoneSending
				SJ 		SendStack
...
;Trap loop "break" byte
WhichLoop:      DB      000h

;Begining of VCAL caller whitelist
;This will grow and needs to be at the tail of the debugger code
whiteList:      DW      00000h
;---------------------------------------------------------------------------
